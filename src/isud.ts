...
export async function store(
  ...
  // l.lib('Start by checking if any of the primary key columns are part of a many-to-one relationship and the referred id is generated by the database')
  // l.lib('If so, insert the rows of the relationship first to be able to populate the columns of the primary key')
  
  // let primaryKey = getPrimaryKey(table)
  // l.lib('Primary key columns', primaryKey)

  // for (let column of primaryKey) {
  //   let relationshipName = getCorrespondingManyToOne(table, column)
  ...
  if (table.relationships) {
    ...
    if (relationshipNames.length > 0) {
      ...
      for (let relationshipName of relationshipNames) {
        ...
        if (typeof relationshipRow != 'object') {
        ...
        if (relationshipRow[relationship.otherId] !== undefined) {
          l.lib(`The id on the relationship row is set. Setting it on the relationship owning row... ${relationship.thisId} = ${relationshipRow[relationship.otherId]}`)
          row[relationship.thisId] = relationshipRow[relationship.otherId]
          continue
        }

        if (! storedRows.isRowAboutToBeStored(relationshipRow)) {
          l.calling('There is a relationship row object and it was not stored yet. Storing it now...')
          ...
          l.called('Stored relationship row...')
          ...
          row[relationship.thisId] = storedRelationshipRow[relationship.otherId]
        }

        else if (relationshipRow[relationship.otherId] !== undefined) {
          l.lib(`Relationship row is about to be stored up the recursion chain but the needed id is there. Setting it... ${relationship.thisId} = ${relationshipRow[relationship.otherId]}`)
          row[relationship.thisId] = relationshipRow[relationship.otherId]
        }
          ...
          storedRows.addAfterStoredRowHandler(relationshipRow, async (relationshipRow: any) => {
            let l = log.fn('afterStoredRowHandler')
            l.param('relationshipRow', relationshipRow)
            ...
            query.set(relationship.thisId, relationshipRow[relationship.otherId])
  ...
  if (table.relationships) {
    for (let relationshipName of Object.keys(table.relationships)) {
      ...
      if (relationship.manyToOne === true && relationship.thisId in row) {
        storeInfo[relationship.thisId] = row[relationship.thisId]
      }
    }
  }
  
  l.lib('Stored row', storeInfo)
  ...
  if (table.relationships) {
    ...
    if (relationshipNames.length > 0) {
      l.lib('Store one-to-many relationship rows', relationshipNames)
      ...
      for (let relationshipName of relationshipNames) {
        ...
        if (relationship.oneToMany !== true) {
          l.lib('Relationship is not one-to-many. Skipping...')
          continue
        }
        ...
        let otherTable = schema[relationship.otherTable]
        if (otherTable == undefined) {
          throw new Error(`Other table '${relationship.otherTable}' defined in relationship '${table}.${relationshipName}' is not contained in the schema.`)
        }
    
        if (otherTable.columns[relationship.otherId] === undefined) {
          throw new Error(`Column '${relationship.otherId}' defined in relationship '${table}.${relationshipName}' is not contained in table '${relationship.otherTable}'.`)
        }
    ...
    for (let relationshipName of relationshipNames) {
      ...
      if (relationship.manyToOne && typeof row[relationshipName] == 'object' && row[relationshipName] !== null) {
        ...
        if (row[relationship.thisId] === undefined) {
          ...
          if (storedRelationshipRow) {
            ...
            if (otherRelationship != undefined) {
              row[relationshipName][otherRelationship.thisId] = storeInfo[otherRelationship.otherId]
            }
            ...
            storeInfo[relationshipName] = storedRelationshipRow
          }
          ...
          for (let column of getPrimaryKey(table)) {
            if (storeInfo[column] == undefined) {
              throw new Error('Some columns of primary are not set.')
            }

            query.where(comparison(column, storeInfo[column]))
          }
        ...
        if (otherRelationship != undefined && storedRelationshipRow != undefined && storedRelationshipRow[otherRelationship.thisId] === undefined) {
          ...
          query.set(otherRelationship.thisId, storeInfo[otherRelationship.otherId])
      ...
      // otherwise we just store the relationship
        ...
        for (let relationshipRow of row[relationshipName]) {
          ...
          if (! storedRows.containsOriginalRow(relationship.otherTable, relationshipRow)) {
            ...
            l.lib('Setting id on relationship row... ' + relationship.otherId + ' = ' + storeInfo[relationship.thisId])
            relationshipRow[relationship.otherId] = storeInfo[relationship.thisId]
            ...
            if (storedRelationshipRow != undefined) {  
              ...
              storeInfo[relationshipName].push(storedRelationshipRow)
            }
  ...
  l.returning('Returning stored row...', storeInfo)
  return storeInfo
}
