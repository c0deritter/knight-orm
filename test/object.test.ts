  ...
  describe('ObjectTools', function() {
    describe('isUpdate', function() {
      it('should return false if the generated primary key is not set', async function() {
        ...
        let result = await objectTools.isUpdate(schema.getTable('table1'), pgQueryFn, row, true)
      ...
      it('should return false if a row with the not generated primary does not exist', async function() {
        ...
        let result = await objectTools.isUpdate(schema.getTable('table2'), pgQueryFn, row, true)
      ...
      it('should return false if a row with the composite primary key does not exist', async function() {
        ...
        let result = await objectTools.isUpdate(schema.getTable('many_to_many_table2'), pgQueryFn, row, true)
    ...
    describe('objectsRepresentSameEntity', function() {
      it('should detect two rows as the same entity', function() {
        ...
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('table1'), row1, row2, true)).to.be.true
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('table1'), row2, row1, true)).to.be.true
  
        let row3 = { table1_id: 1, table2_id: 'x', column1: 'a' }
        let row4 = { table1_id: 1, table2_id: 'x', column1: 'b' }
  
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('many_to_many_table2'), row3, row4, true)).to.be.true
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('many_to_many_table2'), row3, row4, true)).to.be.true
      })
      ...
      it('should not detect two rows as the same entity', function() {
        ...
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('table1'), row1, row2, true)).to.be.false
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('table1'), row2, row1, true)).to.be.false
  
        let row3 = { table1_id: 1, table2_id: 'x' }
        let row4 = { table1_id: 2, table2_id: 'x', column1: 'a' }
  
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('many_to_many_table2'), row3, row4, true)).to.be.false
        expect(objectTools.objectsRepresentSameEntity(schema.getTable('many_to_many_table2'), row3, row4, true)).to.be.false
      })
    })  
  })
